...
export default function Index() {
  ...
  return (
    <div className="mobile-frame bg-background flex flex-col overflow-hidden" style={{ height: "100dvh" }}>
        ...
        <div className="flex items-center justify-between mb-4 animate-fade-up">
          <div>
            <p className="text-xs text-muted-foreground font-medium tracking-widest uppercase">
              {activeNav === 3 ? "Мои покупки" : activeNav === 2 ? "Сохранённые" : "Добро пожаловать"}
            </p>
            <h1 className="text-2xl font-black gradient-text">
              {activeNav === 3 ? "Корзина" : activeNav === 2 ? "Избранное" : "NeonShop"}
            </h1>
          </div>
          <div className="relative">
            <button
              ...
              onClick={() => setActiveNav(activeNav === 3 || activeNav === 2 ? 1 : 3)}
            >
              <Icon name={activeNav === 3 || activeNav === 2 ? "Grid2X2" : "ShoppingBag"} size={20} className="text-foreground" />
            </button>
      ...
      {/* Scrollable content */}
      <div className={`flex-1 overflow-y-auto relative ${activeNav === 3 || activeNav === 2 ? "hidden" : ""}`} style={{ zIndex: 1 }}>
        {/* Categories */}
